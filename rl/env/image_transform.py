"""
Copied version of CUT/data/base_dataset.py modified to perform exactly the same
job but do it only with pytorch tensors (rather than PIL images)
"""
...
def get_transform_2(opt, params=None, grayscale=False, method=InterpolationMode.BICUBIC, convert=True):
    transform_list = []
    ...
    if 'fixsize' in opt.preprocess:
        transform_list.append(transforms.Resize(params["size"], interpolation=method, antialias=True))
